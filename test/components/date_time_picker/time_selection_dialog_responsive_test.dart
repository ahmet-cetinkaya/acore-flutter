...
void main() {
  testWidgets('TimeSelectionDialog responsive mode saves selected time', (WidgetTester tester) async {
    ...
    // Verify dialog is open and in mobile layout (TimePickerMobileContent)
    expect(find.byType(TimePickerMobileContent), findsOneWidget);

    // Find hour wheel and scroll it
    // Initial hour is 10. Scroll to change it.
    // Note: WheelTimePicker uses ListWheelScrollView.
    // We can find the text '10' and drag it.
    await tester.drag(find.text('10'), const Offset(0, -50));
    await tester.pumpAndSettle();

    // Find "Done" button in AppBar (using the key we saw in TimePickerMobileContent)
    ...
    expect(result!.selectedTime.hour, isNot(10)); // Should have changed
  });
}
